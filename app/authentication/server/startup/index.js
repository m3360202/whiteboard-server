...
import { TAPi18n } from 'meteor/universe:i18n';
...
import { Users as UsersRaw } from '../../../models/server/raw/index';
import { addUserRoles } from '../../../../server/webmethods/lib/addUserRoles';
import { getAvatarSuggestionForUser } from '../../../../server/webmethods/lib/getAvatarSuggestionForUser';
import {
  isValidAttemptByUser,
  isValidLoginAttemptByIp,
} from '../lib/restrictLoginAttempts';
import './settings';
import { getClientAddress } from '../../../../server/webmethods/lib/getClientAddress';
import { getNewUserRoles } from '../../../../server/webmethods/lib/getNewUserRoles';
...
const updateMailConfig = _.debounce(() => {
  ...
  Accounts.emailTemplates.from = `${settings.get('Site_Name')} <${settings.get(
    'From_Email',
  )}>`;
}, 1000);
...
Meteor.startup(() => {
  ...
  Accounts.emailTemplates.sitename = 'boarx.us';
  Accounts.emailTemplates.verifyEmail.subject = function (user) {
    return 'Boardx Activation email';
  };
  Accounts.emailTemplates.verifyEmail.html = function (user, url) {
    return (
      'hello, welcome to boardx.us，<br/> please click the below link to activate your account<br/> ' +
      url +
      ' </a><br/>   thank you!'
    );
  };
  Accounts.emailTemplates.resetPassword.subject = function (user) {
    return 'Boardx Reset password Request';
  };
  ...
  Accounts.emailTemplates.resetPassword.html = function (user, url) {
    ...
    // console.log('url', url, 'app url',  process.env.APP_URL, 'root url', process.env.ROOT_URL);
    url=url.replace(root, app);
    return `
      <header>
      <link href='https://fonts.googleapis.com/css?family=sans-serif' rel='stylesheet'>
      <style>
      body {
          font-family: 'sans-serif';font-size: 22px;color:'#000000'
      }
      </style>
      </header>
      <body>
      <img src="https://app.boardx.us/images/logo_1.png"><br /><b />
      <p style=";font-size:36px"> <b>Your password reset request</b> </p>

      <p style=";font-size:16px">Hi ${user.name},</p>  <p>You recently asked us to reset your password. To complete this action, click below </p>
      
      <p style="font-size:16px">
        <a href="${url}">
        <button  style="cursor:pointer; padding: 16px;background: #f21d6b;border-radius: 4px;width:328px;height:56px;border-width:0px;color:#ffffff;font-size:16px ">Reset your password</button>
        </a> 
      </p> 
      </body>`;
  };
  ...
  (insertUserDoc, options, user) => {
    ...
    if (user.username) {
      if (user.type !== 'visitor') {
        Meteor.defer(() => callbacks.run('afterCreateUser', user));
      }
        ...
        Object.keys(avatarSuggestions).some((service) => {
          ...
          if (service !== 'gravatar') {
            Meteor.runAsUser(_id, () =>
              Meteor.call('setAvatarFromService', avatarData.blob, '', service),
            );
            return true;
          }
...
Accounts.validateLoginAttempt((login) => {
  ...
  Meteor.defer(() => callbacks.run('afterValidateLogin', login));
